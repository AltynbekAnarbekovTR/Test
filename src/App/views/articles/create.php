  ...
  <form enctype="multipart/form-data" method="POST" class="grid grid-cols-1 gap-6">
    ...
    <label class="block">
      ...
      <label class="block">
          ...
          <input value="<?php echo e($oldFormData['article_text'] ?? ''); ?>" name="article_text" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
          <?php if (array_key_exists('description', $errors)) : ?>
              <div class="bg-gray-100 mt-2 p-2 text-red-500">
                  <?php echo e($errors['description'][0]); ?>
              </div>
